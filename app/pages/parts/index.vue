    ...
    <section class="panel list-panel parts-list-panel">
      ...
      <div v-if="data?.items.length" class="table-wrap"><table class="parts-table">
        <thead><tr>
          ...
          <th><span class="column-heading">SBE-1</span><select v-model="sbe1" class="column-filter" @change="applyFilters()"><option value="">(All)</option><option v-for="option in data.sbe1Options" :key="option.name" :value="option.name">{{ option.name }} ({{ option.part_count }})</option></select></th>
        ...
        <tbody><tr v-for="part in data.items" :key="part.id">
          ...
          <td><strong>{{ part.sbe1_name || '—' }}</strong></td>
